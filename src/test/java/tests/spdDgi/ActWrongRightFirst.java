...
public class ActWrongRightFirst  {	
	...
	@Test
	public void wrongRightFirst() throws IOException, InterruptedException {
		...
		i.objectTabSwitch();
		obj.addContractTable(correctRightType, "16052016");	
		//==== ДОГОВОР НА ПОМЕЩЕНИЕ (НЕ АРЕНДА) ====//
		i.objectTabSwitch();
		obj.addContractTable(wrongRightType, "16052015");		
		//==== НАРУШЕНИЯ ====//
		i.violTabSwitch();
		viol.addWarning();		
		//==== ВЕРИФИКАЦИЯ ====//
		...
		Props.setProperty("inspIdWrongRightEarly", inspId);
		System.out.println(i.getInspectionId() + " - ДОГОВОР НЕ АРЕНДЫ РАНЬШЕ");
