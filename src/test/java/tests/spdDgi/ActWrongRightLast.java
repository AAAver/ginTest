...
public class ActWrongRightLast  {	
	...
	@Test
	public void wrongRightLast() throws IOException, InterruptedException {
		...
		i.objectTabSwitch();
		obj.addContractTable(wrongRightType, "16052016");	
		//==== ДОГОВОР НА ПОМЕЩЕНИЕ (АРЕНДА) ====//
		i.objectTabSwitch();
		obj.addContractTable(correctRightType, "16052015");				
		//==== НАРУШЕНИЯ ====//
		i.violTabSwitch();
		viol.addWarning();		
		//==== ВЕРИФИКАЦИЯ ====//
//		i.verify();
		...
		Props.setProperty("inspIdWrongRightLast", inspId);
		System.out.println(i.getInspectionId() + " - Договор не аренды был позже");
