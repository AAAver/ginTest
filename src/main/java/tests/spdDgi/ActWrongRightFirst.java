...
public class ActWrongRightFirst extends BaseTest {
	...
	void setDriver() {
		setUpDriver();
		setUpExtentReport("Генерация акта НФ c первым заключенным договором не аренды. is_done = 0");
	}
	...
	@Test(dependsOnMethods = "setEgrnInfo", description = "Первый договор на помещение. Не аренда. Дата раньше")
	void setBuildingContractOne() throws InterruptedException {
		Save.saveThis(driver);
		obj.objectTabSwitch();
		obj.createContractTable(wrongRightType,"16052015");
		obj.objectTabSwitch();
	}
	@Test(dependsOnMethods = "setBuildingContractOne", description = "Второй договор на помещение. Аренда. Дата позже")
	void setBuildingContractTwo() throws InterruptedException {
		Save.saveThis(driver);
		obj.objectTabSwitch();
		obj.createContractTable(correctRightType,"16052016");
		obj.objectTabSwitch();
	}
	...
	void verification() throws InterruptedException {
		...
		log.info("Wrong right contract earlier. is_done=0. ID = " + inspId);
	}
}
